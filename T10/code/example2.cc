...
const double dt_factor_time_dependent = 1;
...
// Calculate acceleration of a given state. This is a procedure which gets a vector of state as an argument.
void calculate_acceleration(vector<state> &here, int i) {
...
void kick(state &here, double mydt) { here.v += here.a * mydt; }
...
void set_timestep(state &here) { here.dt = dt_factor_time_dependent / here.a.abs(); }

int find_next_active(vector<state> &here, double mytime, int myactive) {
  int next_active = myactive;
  double t_min = here[myactive].t + here[myactive].dt - mytime;
#pragma omp parallel for
  for (int i = 0; i < here.size(); i++) {
    if (here[i].t + here[i].dt - mytime < t_min) {
#pragma omp critical
      {
        t_min = here[i].t + here[i].dt - mytime;
        next_active = i;
      }
    }
  }
...
#pragma omp master
  ...
  case 6:
    if (argc < 3)
      initialize_read_xvm(current, "plummer_1000.ascii");
    else
...
#pragma omp parallel for
  for (int i = 0; i < current.size(); i++) {
    calculate_acceleration(current, i);
    set_timestep(current[i]);
  }
  active = find_next_active(current, time, 0);
  ...
  while (current[0].t < TMAX) {
    ...
    if (time >= next_output_time) {
      ...
      } else // make individual files
      {
        ...
        cout << "Writing " << name << " t = " << current[0].t << " Nstep = " << count_steps << endl;
        outfile.open(name);
    ...
    // KDK Scheme:
    kick(current[active], 0.5 * current[active].dt);               // Kick only active particle
    drift(current, current[active].t + current[active].dt - time); // Drift system to next time
    calculate_acceleration(current, active);                       // Acceleration only for active particle
    kick(current[active], 0.5 * current[active].dt);               // Second half kick for active particle
    time = current[active].t + current[active].dt;                 // Advance system time
    current[active].t = time;                                      // Remember time in of active particle
    set_timestep(current[active]);                                 // Calculate new timestep of active particle
    active = find_next_active(current, time, active);              // find next active particle
